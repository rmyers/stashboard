...
from stashboard.content.databases import views
from stashboard.content.databases.workflows import create_instance
...
class DatabaseTests(test.TestCase):
    ...
    def test_launch_instance_exception_on_flavors(self):
        ...
        toSuppress = ["stashboard.content.databases."
    ...
    def _test_details(self, database, test_text, assert_contains=True):
        ...
        # Suppress expected log messages in the test output
        loggers = []
        toSuppress = ["stashboard.content.databases.tabs",
                      "horizon.tables"]
        for cls in toSuppress:
            logger = logging.getLogger(cls)
            loggers.append((logger, logger.getEffectiveLevel()))
